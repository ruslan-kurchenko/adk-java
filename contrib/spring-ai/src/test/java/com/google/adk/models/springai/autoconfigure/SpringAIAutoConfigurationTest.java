...
class SpringAIAutoConfigurationTest {
    ...
    contextRunner
        ...
        .run(
            context -> {
              assertThat(context).hasSingleBean(SpringAI.class);
              SpringAI springAI = context.getBean(SpringAI.class);
              assertThat(springAI.model()).isEqualTo("chat-only-model");
            });
  }
    ...
    contextRunner
        ...
        .run(
            context -> {
              assertThat(context).hasSingleBean(SpringAI.class);
              SpringAI springAI = context.getBean(SpringAI.class);
              assertThat(springAI.model()).isEqualTo("streaming-only-model");
            });
  }
  ...
  @Test
  void testConfigurationValidationEnabled() {
    contextRunner
        .withUserConfiguration(TestConfigurationWithChatModel.class)
        .withPropertyValues(
            "adk.spring-ai.validation.enabled=true",
            "adk.spring-ai.validation.fail-fast=true",
            "adk.spring-ai.temperature=3.0") // Invalid temperature
        .run(
            context -> {
              // With validation enabled and fail-fast true, context should fail to start
              assertThat(context).hasFailed();
              // The validation error is nested deep in the exception stack
              assertThat(context.getStartupFailure())
                  .hasRootCauseInstanceOf(
                      org.springframework.boot.context.properties.bind.validation
                          .BindValidationException.class);
              assertThat(context.getStartupFailure().getMessage()).contains("adk.spring-ai");
            });
  }

  @Test
  void testConfigurationValidationDisabled() {
    contextRunner
        .withUserConfiguration(TestConfigurationWithChatModel.class)
        .withPropertyValues(
            "adk.spring-ai.validation.enabled=false",
            "adk.spring-ai.temperature=1.5") // Valid temperature value
        .run(
            context -> {
              assertThat(context).hasNotFailed();
              assertThat(context).hasSingleBean(SpringAI.class);

              // Verify the validation setting is actually disabled
  ...
  @Test
  void testConfigurationValidationWithFailFastDisabled() {
    contextRunner
        .withUserConfiguration(TestConfigurationWithChatModel.class)
        .withPropertyValues(
            "adk.spring-ai.validation.enabled=true",
            "adk.spring-ai.validation.fail-fast=false",
            "adk.spring-ai.temperature=1.5") // Valid temperature value
        .run(
            context -> {
              assertThat(context).hasNotFailed();
              ...
              // Verify the validation settings
              SpringAIProperties properties = context.getBean(SpringAIProperties.class);
  ...
  @Configuration
  static class TestConfigurationWithStreamingModel {
    @Bean
    public StreamingChatModel streamingChatModel() {
      return prompt ->
          Flux.just(
