...
public class GeminiContextCacheManager {
  ...
  public Single<CacheMetadata> handleContextCaching(LlmRequest llmRequest, int cacheContentsCount) {
    return handleContextCaching(llmRequest, cacheContentsCount, null);
  }
   ...
   * @param llmRequest Request containing cache config and existing metadata
   * @param cacheContentsCount Number of contents to include in cache
   * @param staticInstruction Stable instruction for fingerprinting (excludes dynamic instructions)
   * @return Single emitting updated cache metadata
   */
  public Single<CacheMetadata> handleContextCaching(
      LlmRequest llmRequest, int cacheContentsCount, @Nullable String staticInstruction) {
    ...
    if (canReuseExistingCache(
        existingMetadata, llmRequest, cacheContentsCount, cacheConfig, staticInstruction)) {
      return reuseCache(existingMetadata);
    }

    if (shouldCreateNewCache(existingMetadata, llmRequest, cacheContentsCount, staticInstruction)) {
      return createNewCache(llmRequest, cacheContentsCount, staticInstruction);
    }

    return returnFingerprintOnlyMetadata(llmRequest, cacheContentsCount, staticInstruction);
  }
  ...
  private boolean canReuseExistingCache(
      ...
      @Nullable String staticInstruction) {

    if (metadata == null || !metadata.isActiveCache()) {
      return false;
    }

    if (isCacheValid(metadata, llmRequest, cacheContentsCount, cacheConfig, staticInstruction)) {
      return true;
    }
  ...
  private Single<CacheMetadata> reuseCache(CacheMetadata metadata) {
    ...
    Tracing.recordCacheHit("cache-manager", metadata.cacheName().get());
  ...
  private boolean shouldCreateNewCache(
    ...
    String currentFingerprint =
        generateCacheFingerprint(llmRequest, cacheContentsCount, staticInstruction);
    return shouldCreateFromFingerprintOnly(metadata, currentFingerprint, cacheContentsCount);
  }

  boolean shouldCreateFromFingerprintOnly(
      CacheMetadata metadata, String currentFingerprint, int cacheContentsCount) {
    if (currentFingerprint.equals(metadata.fingerprint())) {
      return true;
    }
    return cacheContentsCount > metadata.contentsCount();
  }
   ...
   *
   * <ul>
   *   <li>Attempts non-blocking lock acquisition with bounded TTL
   *   <li>If lock already held by another pod: returns fingerprint-only metadata (no duplicate
   *       cache creation)
  ...
  private Single<CacheMetadata> createNewCache(
    ...
    logger.debug("Fingerprint-only metadata matches, trying lock for cache creation");
    Tracing.recordCacheMiss("cache-manager");
    ...
    return distributedLock
        ...
        .onErrorResumeNext(
            error -> {
              if (error instanceof DistributedLock.LockAlreadyHeldException) {
                logger.debug(
                    "Lock already held for {}, returning fingerprint-only metadata", lockKey);
                return returnFingerprintOnlyMetadata(
                    llmRequest, cacheContentsCount, staticInstruction);
              }
  ...
  private Single<CacheMetadata> createCache(
    ...
    return Single.fromCallable(
            () -> {
              ...
              int actualContentsCount = Math.min(cacheContentsCount, llmRequest.contents().size());

              CachedContent cachedContent =
                  callGeminiCachingAPI(llmRequest, config, cacheContentsCount);
              String cacheName = cachedContent.name().orElseThrow();
              int cachedTokenCount =
                  cachedContent
                      .usageMetadata()
                      .flatMap(CachedContentUsageMetadata::totalTokenCount)
                      .orElseGet(() -> estimateCachedTokenCount(llmRequest, actualContentsCount));
              long expireTime = calculateExpireTime(config);

              logCacheCreation(cacheName, modelName, actualContentsCount, cachedTokenCount, config);
              warnIfCacheCostIsHigh(cachedTokenCount, config);
              recordCacheCreationMetrics(actualContentsCount);

              return buildCacheMetadata(cacheName, fingerprint, actualContentsCount, expireTime);
            })
  ...
  private CachedContent callGeminiCachingAPI(
    ...
    try {
      ...
      modelName = normalizeModelNameForCaching(modelName);
      // Extract the fully-assembled system instruction (global + static + dynamic).
      // Dynamic instructions are intentionally frozen at cache-creation time: the Gemini API
      // rejects systemInstruction alongside cachedContent.
      ...
      logger.debug(
          "Creating cache for model {} with TTL {}s", modelName, config.ttl().getSeconds());
  ...
  String normalizeModelNameForCaching(String modelName) {
  ...
  private long calculateExpireTime(ContextCacheConfig config) {
    return System.currentTimeMillis() / 1000 + config.ttl().getSeconds();
  }
  ...
  private void logCacheCreation(
    ...
    logger.info(
        "Created cache {} for model {} ({} contents, ~{} tokens, expires in {}s)",
        cacheName,
        modelName,
        cacheContentsCount,
        estimatedTokenCount,
        config.ttl().getSeconds());
  }

  private void warnIfCacheCostIsHigh(int cachedTokenCount, ContextCacheConfig config) {
    if (cachedTokenCount > CACHE_SIZE_CRITICAL_THRESHOLD) {
      logCriticalCostWarning(cachedTokenCount, config);
    } else if (cachedTokenCount > CACHE_SIZE_WARNING_THRESHOLD) {
      logCostInfo(cachedTokenCount, config);
    }
  }

  private void logCriticalCostWarning(int cacheContentsCount, ContextCacheConfig config) {
    double estimatedCost =
        calculateStorageCost(cacheContentsCount, (int) config.ttl().getSeconds());
    logger.warn(
        "Large cache created ({} tokens). Estimated storage cost: ${} for {}h TTL. "
            + "Consider reducing cache size or TTL to minimize costs.",
        cacheContentsCount,
        estimatedCost,
        config.ttl().getSeconds() / 3600.0);
  }
  ...
  private int estimateCachedTokenCount(LlmRequest llmRequest, int cacheContentsCount) {
    int totalChars = String.join("", llmRequest.getSystemInstructions()).length();

    int boundedContentsCount = Math.min(cacheContentsCount, llmRequest.contents().size());
    List<Content> cachedContents = llmRequest.contents().subList(0, boundedContentsCount);
    totalChars += estimateContentChars(cachedContents);

    totalChars += llmRequest.tools().size() * 200;
    if (llmRequest.config().flatMap(GenerateContentConfig::toolConfig).isPresent()) {
      totalChars += 100;
    }
  ...
  private int estimateContentChars(List<Content> contents) {
    int totalChars = 0;
    for (Content content : contents) {
      totalChars += content.role().orElse("").length();
      totalChars +=
          content
              .parts()
              .map(parts -> parts.stream().mapToInt(part -> part.text().orElse("").length()).sum())
              .orElse(0);
    }
    return totalChars;
  }
  ...
  private void recordCacheCreationMetrics(int cacheContentsCount) {
    Tracing.recordCacheCreation("cache-manager", cacheContentsCount);
  }
  ...
  private boolean isCacheValid(
    ...
    if (!isFingerprintMatching(metadata, llmRequest, cacheContentsCount, staticInstruction)) {
    ...
    if (invocationsUsed > cacheConfig.maxInvocations()) {
      logger.info(
          "Cache exceeded invocation limit ({} > {} intervals), will refresh",
          invocationsUsed,
          cacheConfig.maxInvocations());
      return false;
    }
  ...
  public String generateCacheFingerprint(LlmRequest llmRequest, int cacheContentsCount) {
    return generateCacheFingerprint(llmRequest, cacheContentsCount, null);
  }

  String generateCacheFingerprint(
      LlmRequest llmRequest, int cacheContentsCount, @Nullable String staticInstruction) {
    String fingerprintInput =
        buildFingerprintInput(llmRequest, cacheContentsCount, staticInstruction);
  ...
  private String buildFingerprintInput(
      LlmRequest llmRequest, int cacheContentsCount, @Nullable String staticInstruction) {
    StringBuilder input = new StringBuilder();

    appendModelName(llmRequest, input);
    if (staticInstruction != null) {
      input.append("static:").append(staticInstruction).append("|");
    }
    appendSystemInstructions(llmRequest, input);
    appendTools(llmRequest, input);
    appendToolConfig(llmRequest, input);
    appendCodeExecution(llmRequest, input);
  ...
  private void appendToolConfig(LlmRequest llmRequest, StringBuilder input) {
    llmRequest
        .config()
        .flatMap(GenerateContentConfig::toolConfig)
        .ifPresent(toolConfig -> input.append("tool_config:").append(toolConfig).append("|"));
  }

  private void appendCodeExecution(LlmRequest llmRequest, StringBuilder input) {
    boolean hasCodeExecution =
        llmRequest
            ...
            .orElse(false);
    if (hasCodeExecution) {
      input.append("code_execution:true|");
    }
  }
