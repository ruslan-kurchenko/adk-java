...
public class GeminiContextCacheManager {
  ...
  private Single<CacheMetadata> createCache(
    ...
    return Single.fromCallable(
            () -> {
              ...
              String cacheName = callGeminiCachingAPI(llmRequest, fingerprint, config);
  ...
  private String callGeminiCachingAPI(
      LlmRequest llmRequest, String fingerprint, ContextCacheConfig config) {
    try {
      String modelName =
          llmRequest
              .model()
  ...
  private boolean isCacheValid(
    ...
    if (!isContentsCountMatching(metadata, cacheContentsCount)) {
      logger.debug(
  ...
  private boolean isContentsCountMatching(CacheMetadata metadata, int cacheContentsCount) {
    return metadata.contentsCount() == cacheContentsCount;
  }
  ...
  private String buildFingerprintInput(LlmRequest llmRequest, int cacheContentsCount) {
    ...
    appendContentsCount(cacheContentsCount, input);
  ...
  private void appendContentsCount(int cacheContentsCount, StringBuilder input) {
    input.append("contents_count:").append(cacheContentsCount);
  }
