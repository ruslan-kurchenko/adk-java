...
public class InMemoryDistributedLock implements DistributedLock {
  ...
  /**
   * Default lock TTL for the blocking {@link #acquire} path. This is intentionally longer than the
   * acquisition wait timeout to ensure the lock does not expire while the holder is still executing
   * the critical section (e.g., a slow Gemini API call).
   */
    ...
    return Single.create(
        emitter -> {
          ...
          while (System.currentTimeMillis() - startTime < timeoutMs) {
            if (tryAcquireInternal(lockKey, lockValue, DEFAULT_LOCK_TTL)) {
    ...
    public Completable extend(Duration additionalTime) {
      return Completable.fromAction(
          () -> {
            ...
            // Extend from existing expiry to prevent shortening under early extension
            long newExpiryTime = entry.expiryTime + additionalTime.toMillis();
