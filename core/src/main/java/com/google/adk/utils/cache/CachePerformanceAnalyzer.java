...
record CacheStatistics(int cacheRefreshCount, String latestCacheName) {}
...
public class CachePerformanceAnalyzer {
  ...
  private CacheStatistics calculateCacheStatistics(List<CacheMetadata> cacheHistory) {
    int cacheRefreshCount = countCacheRefreshes(cacheHistory);
    String latestCacheName = cacheHistory.get(cacheHistory.size() - 1).cacheName().orElse("");

    return new CacheStatistics(cacheRefreshCount, latestCacheName);
  }

  private int countCacheRefreshes(List<CacheMetadata> cacheHistory) {
    String previousCacheName = null;
    int refreshCount = 0;

    for (CacheMetadata metadata : cacheHistory) {
      String cacheName = metadata.cacheName().orElse("");
      ...
      if (!previousCacheName.equals(cacheName)) {
        refreshCount++;
        previousCacheName = cacheName;
      }
    }
