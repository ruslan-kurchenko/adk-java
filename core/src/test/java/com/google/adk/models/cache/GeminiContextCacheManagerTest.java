...
public class GeminiContextCacheManagerTest {
  ...
  @Test
  public void generateCacheFingerprint_withStaticInstruction_stillVariesByFullSystemInstruction() {
    LlmRequest request1 =
        LlmRequest.builder()
            .model(TEST_MODEL)
            .config(
    ...
    String fingerprint1 =
        manager.generateCacheFingerprint(request1, 5, "shared-static-instruction");
    String fingerprint2 =
        manager.generateCacheFingerprint(request2, 5, "shared-static-instruction");

    assertThat(fingerprint1).isNotEqualTo(fingerprint2);
  }

  @Test
  public void generateCacheFingerprint_withSameToolConfig_isDeterministic() {
    ToolConfig toolConfigA =
        ToolConfig.builder()
            .functionCallingConfig(
                FunctionCallingConfig.builder()
                    .mode(FunctionCallingConfigMode.Known.ANY)
                    .allowedFunctionNames("getWeather", "bookFlight")
                    .build())
            .build();

    ToolConfig toolConfigB =
        ToolConfig.builder()
            .functionCallingConfig(
                FunctionCallingConfig.builder()
                    .mode(FunctionCallingConfigMode.Known.ANY)
  ...
  @Test
  public void generateCacheFingerprint_differentToolConfig_returnsDifferentHashes() {
    ToolConfig toolConfigA =
        ToolConfig.builder()
            .functionCallingConfig(
                FunctionCallingConfig.builder()
    ...
    ToolConfig toolConfigB =
        ToolConfig.builder()
            .functionCallingConfig(
                FunctionCallingConfig.builder().mode(FunctionCallingConfigMode.Known.NONE).build())
            .build();
    ...
    String fingerprintA = manager.generateCacheFingerprint(requestA, 5);
    String fingerprintB = manager.generateCacheFingerprint(requestB, 5);
  ...
  @Test
  public void shouldCreateFromFingerprintOnly_matchingFingerprint_returnsTrue() {
    CacheMetadata fingerprintOnly =
        CacheMetadata.builder().fingerprint("existing-fingerprint").contentsCount(4).build();

    boolean shouldCreate =
        manager.shouldCreateFromFingerprintOnly(fingerprintOnly, "existing-fingerprint", 4);

    assertThat(shouldCreate).isTrue();
  }

  @Test
  public void shouldCreateFromFingerprintOnly_contentsGrowthWithoutMatch_returnsTrue() {
    CacheMetadata fingerprintOnly =
        CacheMetadata.builder().fingerprint("old-fingerprint").contentsCount(4).build();

    boolean shouldCreate =
        manager.shouldCreateFromFingerprintOnly(fingerprintOnly, "new-fingerprint", 6);

    assertThat(shouldCreate).isTrue();
  }

  @Test
  public void shouldCreateFromFingerprintOnly_noGrowthAndNoMatch_returnsFalse() {
    CacheMetadata fingerprintOnly =
        CacheMetadata.builder().fingerprint("old-fingerprint").contentsCount(6).build();
  ...
  @Test
  public void normalizeModelNameForCaching_shortModel_addsModelsPrefix() {
    String normalized = manager.normalizeModelNameForCaching("gemini-2.5-flash");

    assertThat(normalized).isEqualTo("models/gemini-2.5-flash");
  }

  @Test
  public void normalizeModelNameForCaching_prefixedModel_keepsValue() {
    String normalized = manager.normalizeModelNameForCaching("models/gemini-2.5-flash");

    assertThat(normalized).isEqualTo("models/gemini-2.5-flash");
  }

  @Test
  public void normalizeModelNameForCaching_fullyQualifiedModel_keepsValue() {
    String fullyQualified =
        "projects/test/locations/us-central1/publishers/google/models/gemini-2.5-flash";

    String normalized = manager.normalizeModelNameForCaching(fullyQualified);

    assertThat(normalized).isEqualTo(fullyQualified);
  }
  ...
  @Test
  public void generateCacheFingerprint_withFullyQualifiedVertexModel_doesNotPrependModels() {
    String vertexModel =
        "projects/my-project/locations/us-central1/publishers/google/models/gemini-1.5-flash-001";
    LlmRequest request = LlmRequest.builder().model(vertexModel).build();

    String fingerprint = manager.generateCacheFingerprint(request, 0);

    LlmRequest requestWithPrepended = LlmRequest.builder().model("models/" + vertexModel).build();
    String fingerprintWithPrepended = manager.generateCacheFingerprint(requestWithPrepended, 0);

    assertThat(fingerprint).isNotEqualTo(fingerprintWithPrepended);
  }
}
