...
public class ContextCacheConfigTest {
  ...
  public void builder_createsConfigWithCustomValues() {
    ContextCacheConfig config =
        ContextCacheConfig.builder().cacheIntervals(20).ttlSeconds(3600).minTokens(1000).build();

    assertThat(config.cacheIntervals()).isEqualTo(20);
  ...
  public void builder_acceptsMinimumValidValues() {
    ContextCacheConfig config =
        ContextCacheConfig.builder().cacheIntervals(1).ttlSeconds(1).minTokens(0).build();

    assertThat(config.cacheIntervals()).isEqualTo(1);
  ...
  public void builder_acceptsMaximumValidValues() {
    ...
    assertThat(config.cacheIntervals()).isEqualTo(100);
    assertThat(config.ttlSeconds()).isEqualTo(Integer.MAX_VALUE);
    assertThat(config.minTokens()).isEqualTo(Integer.MAX_VALUE);
  }

  @Test
  public void build_throwsException_whenCacheIntervalsTooLow() {
    ContextCacheConfig.Builder builder = ContextCacheConfig.builder().cacheIntervals(0);

    IllegalArgumentException exception =
        assertThrows(IllegalArgumentException.class, builder::build);

    assertThat(exception).hasMessageThat().contains("cacheIntervals must be between 1 and 100");
    assertThat(exception).hasMessageThat().contains("got: 0");
  }

  @Test
  public void build_throwsException_whenCacheIntervalsTooHigh() {
    ContextCacheConfig.Builder builder = ContextCacheConfig.builder().cacheIntervals(101);

    IllegalArgumentException exception =
        assertThrows(IllegalArgumentException.class, builder::build);

    assertThat(exception).hasMessageThat().contains("cacheIntervals must be between 1 and 100");
    assertThat(exception).hasMessageThat().contains("got: 101");
  }

  @Test
  public void build_throwsException_whenCacheIntervalsNegative() {
    ContextCacheConfig.Builder builder = ContextCacheConfig.builder().cacheIntervals(-1);
  ...
  public void toString_returnsFormattedString() {
    ContextCacheConfig config =
        ContextCacheConfig.builder().cacheIntervals(15).ttlSeconds(3600).minTokens(5000).build();

    String result = config.toString();

    assertThat(result).contains("cacheIntervals=15");
  ...
  public void toString_includesAllFields() {
    ...
    assertThat(result).contains("cacheIntervals=");
  ...
  @Test
  public void equals_returnsFalseForDifferentCacheIntervals() {
    ContextCacheConfig config1 = ContextCacheConfig.builder().cacheIntervals(10).build();

    ContextCacheConfig config2 = ContextCacheConfig.builder().cacheIntervals(20).build();
  ...
  public void builder_supportsMethodChaining() {
    ContextCacheConfig config =
        ContextCacheConfig.builder().cacheIntervals(25).ttlSeconds(7200).minTokens(2000).build();

    assertThat(config.cacheIntervals()).isEqualTo(25);
